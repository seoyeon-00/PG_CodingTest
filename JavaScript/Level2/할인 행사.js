...
|     ["banana", "apple",     | [3, 2, 2, 2, 1] | "apple", "pork", "banana", "pork", "rice", "pot",|   3   |
|    "rice", "pork", "pot"]   |                 |            "banana", "apple", "banana"]          |       |
|-----------------------------|-----------------|--------------------------------------------------|-------|
|                             |     	          |     ["banana", "banana", "banana", "banana",     |       |
|          ["apple"]          |     	[10]      |      "banana", "banana", "banana", "banana",     |   0   |
|                             |                 |                 "banana", "banana"]              |       |
*/
...
function solution(want, number, discount) {
  ...
  for (let i = 0; i < discount.length; i++) {
    // 10일 동안 할인하는 상품 목록 리스트
    const discountDay = discount.slice(i, i + 10);
    let check = 0; // 리스트에 원하는 상품의 조건이 만족하다면 +1

    // 할인 리스트 길이가 10이면 조건문 실행
    if (discountDay.length === 10) {
      // want 요소가 discountDay에 몇 번 포함되었는지 확인.
      want.forEach((item, idx) => {
        let arr = discountDay.filter((ele) => ele === item).length;
        if (arr === number[idx]) check++;
      });
    } else {
      break; // 반복문 종료 (10일 동안 회원 자격 가질 수 없음)
    }

    // check와 want의 수가 일치하면 answer +1
