...
조건에 맞는 소수는 왼쪽부터 순서대로 211, 2, 11이 있으며, 총 3개입니다. 
...
function solution(n, k) {
  let answer = 0;
  const num = k === 10 ? parseInt(n, 10).toString() : n.toString(k);
  ...
  // 0을 기준으로 문자열을 잘라 배열로 반환, "" 원소는 제거
  const filterArr = num.split("0").filter((item) => item !== "");

  // 배열을 순회하여 원소가 소수일 때 +answer
  for (var i of filterArr) {
    if (isPrime(i)) answer++;
  }
