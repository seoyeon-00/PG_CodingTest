...
function solution(s) {
  ...
  // 문자열 s의 중괄호를 대괄호로 변환
  let parenthesesChange = s.replaceAll("{", "[").replaceAll("}", "]");

  // 문자열을 배열로 변환
  const strArr = JSON.parse(parenthesesChange);

  // 2차원 배열 배열의 길이를 기준으로 정렬
  strArr.sort((a, b) => a.length - b.length);

  // strArr 순회 answer에 strArr[i]에 대한 원소가 포함하지 않는다면 answer에 추가
  for (let i = 0; i < strArr.length; i++) {
    strArr[i].forEach((item) => {
