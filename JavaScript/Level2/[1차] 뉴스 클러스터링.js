/*
[문제]
뉴스 클러스터링 여러 언론사에서 쏟아지는 뉴스, 특히 속보성 뉴스를 보면 
비슷비슷한 제목의 기사가 많아 정작 필요한 기사를 찾기가 어렵다. 
...
function solution(str1, str2) {
  // 문자열을 2글자씩 영문으로 된 쌍을 배열로 리턴
  function splitStr(str) {
    let arr = [];

    for (let i = 0; i < str.length; i++) {
      const addStr = (str[i] + str[i + 1]).toLowerCase();
      const regex = /[^a-zA-Z]/g;

      if (str[i + 1] && !regex.test(addStr)) {
        arr.push(addStr);
      }
    }
    return arr;
  ...
  // str1, str2 영문 2글자  Array
  let str1Arr = splitStr(str1);
  let str2Arr = splitStr(str2);

  // 두 배열의 원소 합
  const allEle = str1Arr.length + str2Arr.length;
  let commonEle = 0; // 교집합

  // str1Arr 원소를 순회하여 str2Arr에 해당 원소가 존재하면 해당 원소 제거 후 commonEle +1
  str1Arr.forEach((item, _) => {
    if (str2Arr.includes(item)) {
      const findRemoveIndex = str2Arr.findIndex((ele) => ele === item);
      str2Arr.splice(findRemoveIndex, 1);
      commonEle++;
    }
  });
  ...
  return allEle === 0
    ? 65536
    : Math.floor((commonEle / (allEle - commonEle)) * 65536);
}
