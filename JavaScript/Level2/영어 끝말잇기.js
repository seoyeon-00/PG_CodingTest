...
function solution(n, words) {
  ...
  for (let i = 0; i < words.length; i++) {
    // 이전에 나왔던 배열들이 현재 words[i]에 포함되지 않는다면
    if (!words.slice(0, i).includes(words[i])) {
      // words[i + 1]이 존재하고, words[i]의 뒷 글자와 다음 단어의 앞글자가 일치하지 않을 경우
      if (words[i + 1] && words[i][words[i].length - 1] !== words[i + 1][0]) {
        answer += words[i + 1];
        break;
      }
    } else {
      // 이전에 나왔던 배열들이 현재 words[i]에 포함
      answer += words[i];
      break;
    }
  }
  ...
  // answer의 위치를 알아냄
  var findIndex = words.lastIndexOf(answer);

  // answer가 빈 문자열이면 [0,0]
  return answer !== ""
    ? [(findIndex % n) + 1, Math.floor(findIndex / n) + 1]
    : [0, 0];
}
