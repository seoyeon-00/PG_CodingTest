...
|                  record                   |                         result                        |
|-------------------------------------------|-------------------------------------------------------|
|  ["Enter uid1234 Muzi", "Enter uid4567    |  ["Prodo님이 들어왔습니다.", "Ryan님이 들어왔습니다.",  |
|   Prodo","Leave uid1234","Enter uid1234   |   "Prodo님이 나갔습니다.", "Prodo님이 들어왔습니다."]   |
...
function solution(record) {
  ...
  // 객체 obj에 아이디에 대하여 최근에 사용하고 있는 닉네임을 저장
  const obj = {};
  record.forEach((item) => {
    const splitItem = item.split(" "); // 문자열을 " " 기준으로 배열로 쪼갬
    if (splitItem[0] === "Enter" || splitItem[0] === "Change") {
      obj[splitItem[1]] = splitItem[2];
    }
  });

  // record를 순회. 최신 닉네임으로 메시지 출력
  for (let i = 0; i < record.length; i++) {
    const splitItem = record[i].split(" ");
    if (splitItem[0] === "Enter") {
      const str = obj[splitItem[1]] + "님이 들어왔습니다.";
      answer.push(str);
    } else if (splitItem[0] === "Leave") {
      const str = obj[splitItem[1]] + "님이 나갔습니다.";
      answer.push(str);
    }
  }
