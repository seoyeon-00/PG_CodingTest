...
function solution(priorities, location) {
  let answer = 0;
  const prior = priorities.map((priority, idx) => [priority, idx]);
  const arr = [];

  while (prior.length !== 0) {
    // prior의 첫번째 원소 제거 및 저장
    let cur = prior.shift();

    // 현재 원소의 우선순위가 모든 원소의 우선순위보다 작다면 다시 큐에 넣음
    if (prior.some((ele) => cur[0] < ele[0])) {
      prior.push(cur);
    } else {
      arr.push(cur); // 크다면 arr에 저장
      answer++;
      // arr의 idx가 location과 같다면 answer 리턴
      if (arr[arr.length - 1][1] === location) {
        return answer;
      }
    }
  }
